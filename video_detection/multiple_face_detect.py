...
def start_app(cnn):
    ...
    while cap.isOpened():
        ...
        threshold_index1 = int(
            width / 3
        )  # so far just sliced into three same-sized areas --> has to be adapted to fit the use case
        ...
        area2_faces = [
            ...
            and face.x + face.w / 2 < threshold_index2
        ]
        area3_faces = [
            face for face in faces if face.x + face.w / 2 >= threshold_index2
        ]
        ...
        for i in range(3):
            if not area_faces[i]:
                continue
            max_prob_face = max(
                area_faces[i], key=lambda x: x.w * x.h
            )  # assuming w*h (area of face rectangle) as the proxy for face probability
        ...
        for i in range(3):
            ...
            if expiry_timers[i] is not None:
                cv2.putText(
                    fr,
                    f"Expiry: {int(expiry_timers[i] - current_time)}",
                    (threshold_index, 50),
                    fontFace=font,
                    fontScale=1,
                    color=(0, 0, 0),
                ...
                cv2.putText(
                    fr,
                    f"Timer: {int(current_time - face_timers[i])}",
                    (threshold_index, 50),
                    fontFace=font,
                    fontScale=1,
                    color=(0, 0, 0),
